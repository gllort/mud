...
****		   .  Modifica la gestion de errores de db_exec para 
****		      estandarizar los valores de retorno. 
****		      Ver documentacion complementaria.
****		   .  Nueva funcion db_affected_rows() para MySQL (tested) 
****		      y mSQL (No esta probado, aunque deberia funcionar sin 
****		      problemas. En caso contrario, contacta conmigo!)	
****		   .  Nueva funcion db_errno() para MySQL.
****		   .  Nueva funcion db_error() para MySQL y mSQL.
****		   .  db_commit() y db_rollback() funcionales para MySQL.
****
****	    Feb 2003:
****		Cain@MundoOscuro
****		   .  Added db_insert_id() for MySQL.
****			
****    Notes:
...
static int	msql_affected_rows PROT((dbconn_t *));

static db_defn_t msql = {
    "mSQL", msql_connect, msql_close, msql_execute, msql_fetch, NULL, NULL, msql_cleanup, NULL, msql_errormsg, msql_affected_rows, NULL, NULL
};
#endif
...
static int	MySQL_commit   PROT((dbconn_t *));
static int	MySQL_rollback PROT((dbconn_t *));
static void     MySQL_cleanup  PROT((dbconn_t *));
static char *   MySQL_errormsg PROT((dbconn_t *));
static int	MySQL_affected_rows PROT((dbconn_t *));
static int	MySQL_errnum   PROT((dbconn_t *));
static int	MySQL_insert_id PROT((dbconn_t *));

static db_defn_t mysql = {
    "MySQL", MySQL_connect, MySQL_close, MySQL_execute, MySQL_fetch, MySQL_commit, MySQL_rollback, MySQL_cleanup, NULL, MySQL_errormsg, MySQL_affected_rows, MySQL_errnum, MySQL_insert_id
};
#endif
 ...
 *
 * Returns number of rows in result set on success, -1 on failure
...
void f_db_rollback PROT((void))
{
    ...
    if (ret >= 0) {
	//Cuando se ejecuta rollback correctamente
	if (db->type->cleanup) {
	    db->type->cleanup(&(db->c));
	}
    }
...
#ifdef F_DB_AFFECTED_ROWS
void f_db_affected_rows PROT((void)) {
    db_t * db;
    db = find_db_conn(sp->u.number);
    if (!db) {
        error("Attemp to query affected rows on invalid database handle\n"); 
    }
    sp->u.number = db->type->affected_rows(&(db->c));
}
#endif

#ifdef USE_MYSQL
static int MySQL_affected_rows P1(dbconn_t *, c) {
    return mysql_affected_rows(c->mysql.handle);
}
#endif

#ifdef USE_MSQL
static int msql_affected_rows P1(dbconn_t *, c) {
    //En versiones anteriores a mSQL 2, siempre sera 0.
    return c->msql.affected_rows;
}
#endif

#ifdef F_DB_ERRNO
void f_db_errno PROT((void)) {
    db_t * db;
    db = find_db_conn(sp->u.number);
    if (!db) {
        error("Attemp to query error number on invalid database handle\n"); 
    }
    sp->u.number = db->type->errnum(&(db->c));
}
#endif

#ifdef USE_MYSQL
static int MySQL_errnum P1(dbconn_t *, c) {
    return mysql_errno(c->mysql.handle);
}
#endif

#ifdef F_DB_ERROR
void f_db_error PROT((void)) {
    db_t * db;
    db = find_db_conn(sp->u.number);
    if (!db) {
        error("Attemp to query error on invalid database handle\n"); 
    }
    if (db->type->error) {
        char *errormsg;
        ...
        put_constant_string("Error desconocido");
    }
}
#endif

#ifdef USE_MYSQL
static int MySQL_commit P1(dbconn_t *, c) {
    return MySQL_execute(c, "COMMIT");
}
#endif

#ifdef USE_MYSQL
static int MySQL_rollback P1(dbconn_t *, c) {
    return MySQL_execute(c, "ROLLBACK");
}
#endif

#ifdef F_DB_INSERT_ID
void f_db_insert_id PROT((void)) {
    db_t * db;
    db = find_db_conn(sp->u.number);
    if (!db) {
        error("Attemp to query last inserted id on invalid database handle\n"); 
    }
    sp->u.number = db->type->insert_id(&(db->c));
}
#endif

#ifdef USE_MYSQL
static int MySQL_insert_id P1(dbconn_t *, c) {
    return mysql_insert_id(c->mysql.handle);
}
#endif
