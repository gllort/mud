...
** 07-Sep-03. Cain. Traducido y adaptado a Mundo Oscuro.
*/

#include <mudlib.h>
#include <sockets.h>
#include <sockets_errno.h>
#include <log.h>

//#define SKTLOG(x,y)	log(LOG_SOCKET, sprintf("%s: %O\n",x,y))
#define SKTLOG(x,y)

nosave private int	style;
nosave private int	fdOwned = -1;	/* El socket aun no esta creado */
...
int debug() {
	...
	case SOCKET_LISTEN_M:
	    printf("%O: (mud) listening at %O\n", this_object(), addr);
	    printf("    read_func=%O  close_func=%O\n", read_func, close_func);
	    break;

	case SOCKET_CONNECT_M:
...
SKTLOG("read_callback: message", message);
...
SKTLOG("listen_callback: fd",fd);
    ...
    s = new(SOCKET, SOCKET_ACQUIRE, read_func, close_func);
...
SKTLOG("create: SOCKET_LISTEN",fdOwned);
	break;
...
*/
	...
	if ( err < 0 )
	    error("could not listen to socket: " + socket_error(err) + "\n");
SKTLOG("create: SOCKET_CONNECT",fdOwned);
SKTLOG("create: close_func",close_func);
	break;

    case SOCKET_LISTEN_B:
        read_func = p2;
        close_func = p3;
        ...
        if ( (err = socket_listen(fdOwned, "listen_callback")) < 0 )
            error("could not listen to socket: " + socket_error(err) + "\n");
SKTLOG("create: SOCKET_LISTEN_B",fdOwned);
        break;

    case SOCKET_CONNECT_B:
/*
#ifdef REQUIRE_PRIV
	if ( !check_previous_privilege(REQUIRE_PRIV) )
	{
...
*/
        ...
        if ( err < 0 )
            error("could not listen to socket: " + socket_error(err) + "\n");
SKTLOG("create: SOCKET_CONNECT_B",fdOwned);
SKTLOG("create: close_func",close_func);
        break;
...
*/
	...
	if ( (err = socket_bind(fdOwned, p1)) < 0 )
	    error("could not bind socket: " + socket_error(err) + "\n");
SKTLOG("create: SOCKET_UDP",fdOwned);
	break;
	...
	if ( (err = socket_listen(fdOwned, "listen_callback")) < 0 )
	    error("could not listen to socket: " + socket_error(err) + "\n");
SKTLOG("create: SOCKET_LISTEN_M",fdOwned);
	break;
...
*/
	...
	if ( err < 0 )
	    error("could not listen to socket: " + socket_error(err) + "\n");
SKTLOG("create: SOCKET_CONNECT_M",fdOwned);
SKTLOG("create: close_func",close_func);
	break;
